...
router.get('/instances', authenticate, requireManager, async (req, res) => {
  try {
    if (!req.user) {
...
router.post('/instances', authenticate, requireManager, logAction('create_whatsapp_instance'), async (req, res) => {
  try {
...
router.get('/instances/:instanceId', authenticate, async (req, res) => {
  try {
...
router.patch('/instances/:instanceId/status', authenticate, async (req, res) => {
  try {
    const instanceId = parseInt(req.params.instanceId);
    ...
    if (phone_number) additionalData.phone_number = phone_number;
    if (qr_code !== undefined) additionalData.qr_code = qr_code;
    if (session_data) additionalData.session_data = session_data;
    if (status === 'connected') additionalData.connected_at = new Date();
    
    const success = await WhatsAppInstanceModel.updateStatus(instanceId, status, additionalData);
...
router.post('/instances/:instanceId/disconnect', authenticate, logAction('disconnect_whatsapp'), async (req, res) => {
  try {
    const instanceId = parseInt(req.params.instanceId);
    ...
    const success = await WhatsAppInstanceModel.disconnect(instanceId);
...
// Deletar instância
router.delete('/instances/:instanceId', authenticate, logAction('delete_whatsapp_instance'), async (req, res) => {
  try {
    const instanceId = parseInt(req.params.instanceId);
    ...
    const success = await WhatsAppInstanceModel.deactivate(instanceId);
...
router.get('/stats', authenticate, requireManager, async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      stats = await WhatsAppInstanceModel.getStats();
    } else {
      // Gestor vê apenas suas estatísticas
      stats = await WhatsAppInstanceModel.getStats(req.user.id);
    }
...
router.post('/maintenance/clear-qr', authenticate, requireManager, async (req, res) => {
  try {
    const clearedCount = await WhatsAppInstanceModel.clearExpiredQRCodes();
    res.json({ 
...
router.post('/instances/:instanceId/heartbeat', authenticate, async (req, res) => {
  try {
    const instanceId = parseInt(req.params.instanceId);
    ...
    const success = await WhatsAppInstanceModel.updateActivity(instanceId);
...
router.get('/instances/status', authenticate, async (req: any, res) => {
  try {
    const managerId = req.user.id

    const [instances] = await pool.execute<RowDataPacket[]>(
      'SELECT id, instance_name, status, phone_number, connected_at, last_activity FROM whatsapp_instances WHERE manager_id = ? AND is_active = 1',
      [managerId]
    )

    const totalInstances = instances.length
    const connectedInstances = instances.filter((instance: any) => instance.status === 'connected').length
...
router.get('/chats/pending-count', authenticate, async (req: any, res) => {
  try {
    const managerId = req.user.id

    // Buscar conversas pendentes
    const [pendingChats] = await pool.execute<RowDataPacket[]>(
      'SELECT COUNT(*) as count FROM human_chats WHERE manager_id = ? AND status = "pending"',
      [managerId]
    )

    // Buscar conversas ativas sem operador
    const [unassignedChats] = await pool.execute<RowDataPacket[]>(
      'SELECT COUNT(*) as count FROM human_chats WHERE manager_id = ? AND status = "active" AND operator_id IS NULL',
      [managerId]
    )
