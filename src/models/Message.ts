...
export class MessageModel {
  ...
  static async checkDuplicate(data: CreateMessageData): Promise<Message | null> {
    ...
    if (data.whatsapp_message_id) {
      const query = `
        ...
        LIMIT 1
      `;
      const result = await executeQuery(query, [data.whatsapp_message_id, data.manager_id]) as Message[];
  ...
  static async findByChat(chatId: number, limit: number = 50): Promise<Message[]> {
    ...
    try {
      ...
      const chatResult = await executeQuery(chatQuery, [safeChatId]);
      ...
      const result = await executeQuery(query, [contactId, managerId, safeChatId, safeLimit]);
      ...
      console.log(`✅ Debug findByChat - Mensagens encontradas:`, result.map((m: any) => ({
        id: m.id,
