...
import { executeQuery, getPoolStats, ensurePoolConnection } from '../config/database';
...
async function testDatabaseConnection() {
  ...
  try {
    ...
    const result1 = await executeQuery('SELECT NOW() as current_time, VERSION() as mysql_version');
    console.log('✅ Query simples executada:', result1[0]);
    
    // Teste 2: Query com parâmetros
    console.log('\n2️⃣ Testando query com parâmetros...');
    const result2 = await executeQuery('SELECT ? as test_param, ? as test_number', ['teste', 123]);
    ...
    try {
      const result3 = await executeQuery('SELECT COUNT(*) as total_users FROM users');
