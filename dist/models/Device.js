...
class DeviceModel {
    ...
    static async findByFingerprint(managerId, fingerprint) {
        const query = 'SELECT * FROM devices WHERE manager_id = ? AND device_fingerprint = ?';
        const result = await (0, database_1.executeQuery)(query, [managerId, fingerprint]);
        if (!Array.isArray(result) || result.length === 0) {
            return null;
        }
        return DeviceModel.parseDevice(result[0]);
    ...
    static async setPrimary(id, managerId) {
        // Primeiro, remover o flag primário de todos os dispositivos do gestor
        await (0, database_1.executeQuery)('UPDATE devices SET is_primary = FALSE WHERE manager_id = ?', [managerId]);
        // Depois, definir o dispositivo atual como primário
        const result = await (0, database_1.executeQuery)('UPDATE devices SET is_primary = TRUE, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND manager_id = ?', [id, managerId]);
        return result.affectedRows > 0;
    }
    // Confiar no dispositivo
    static async setTrusted(id, trusted) {
        const query = `
      UPDATE devices 
      SET is_trusted = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `;
        const result = await (0, database_1.executeQuery)(query, [trusted, id]);
        return result.affectedRows > 0;
    }
    // Bloquear/desbloquear dispositivo
    static async setBlocked(id, blocked) {
    ...
    static async findOnlineDevices(managerId) {
        let query = `
      SELECT * FROM devices 
      WHERE status = 'online'
    `;
    ...
    static async markInactiveAsOffline() {
        const query = `
      UPDATE devices 
      SET status = 'offline', updated_at = CURRENT_TIMESTAMP
    ...
    static async findPrimaryDevice(managerId) {
        const query = `
      SELECT * FROM devices 
    ...
    static async findWithWhatsAppInstances(managerId) {
        let query = `
    ...
    static async linkToWhatsAppInstance(deviceId, instanceId) {
        const query = `
      UPDATE devices 
      SET whatsapp_instance_id = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `;
        const result = await (0, database_1.executeQuery)(query, [instanceId, deviceId]);
        return result.affectedRows > 0;
    }
    // Desassociar dispositivo de instância WhatsApp
    static async unlinkFromWhatsAppInstance(deviceId) {
        const query = `
      UPDATE devices 
    ...
    static async updateWhatsAppStatus(deviceId, status, phone) {
    ...
    static async canDelete(deviceId) {
    ...
    static async syncWithWhatsAppInstances() {
        const query = `
      UPDATE devices d
      LEFT JOIN whatsapp_instances wi ON d.whatsapp_instance_id = wi.id
      SET 
    ...
    static async autoCreateForWhatsAppInstances() {
    ...
    `;
        const orphanInstances = await (0, database_1.executeQuery)(orphanQuery);
