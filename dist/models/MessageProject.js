...
exports.AutoMessageModel = exports.MessageProjectModel = void 0;
const database_1 = require("../config/database");
class MessageProjectModel {
    // Criar projeto
    static async create(data) {
        // Se este projeto deve ser padrão, primeiro remover padrão dos outros
        if (data.is_default) {
            await (0, database_1.executeQuery)('UPDATE message_projects SET is_default = FALSE WHERE manager_id = ?', [data.manager_id]);
        }
        const query = `
    ...
    static async findById(id, includeMessages = false) {
    ...
    static async findByManagerId(managerId, includeMessages = false) {
        const query = `
    ...
    static async findDefaultByManagerId(managerId, includeMessages = true) {
        const query = `
      SELECT * FROM message_projects 
    ...
    static async update(id, updateData) {
        ...
        if (updateData.is_default !== undefined) {
            ...
            if (updateData.is_default) {
                ...
                if (project) {
                    await (0, database_1.executeQuery)('UPDATE message_projects SET is_default = FALSE WHERE manager_id = ? AND id != ?', [project.manager_id, id]);
                }
            }
    ...
    static async setAsDefault(projectId) {
        ...
        await (0, database_1.executeQuery)('UPDATE message_projects SET is_default = FALSE WHERE manager_id = ?', [project.manager_id]);
        // Definir este projeto como padrão
        await (0, database_1.executeQuery)('UPDATE message_projects SET is_default = TRUE WHERE id = ?', [projectId]);
        return this.findById(projectId);
    }
    // Deletar projeto
    static async delete(projectId) {
        const query = `DELETE FROM message_projects WHERE id = ?`;
        await (0, database_1.executeQuery)(query, [projectId]);
    }
}
exports.MessageProjectModel = MessageProjectModel;
class AutoMessageModel {
    ...
    `;
        const result = await (0, database_1.executeQuery)(query, [projectId]);
        if (!Array.isArray(result)) {
            return [];
        }
    ...
    static async reorder(projectId, messageIds) {
        try {
            for (let i = 0; i < messageIds.length; i++) {
                await (0, database_1.executeQuery)('UPDATE auto_messages SET order_index = ? WHERE id = ? AND project_id = ?', [i, messageIds[i], projectId]);
            }
            return true;
        }
        catch (error) {
...
exports.AutoMessageModel = AutoMessageModel;
//# sourceMappingURL=MessageProject.js.map