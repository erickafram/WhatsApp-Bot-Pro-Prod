...
router.get('/', auth_1.authenticate, requireOperatorAccess, async (req, res) => {
    try {
      ...
      ORDER BY u.is_active DESC, u.created_at DESC`, [managerId]);
...
router.get('/:id', auth_1.authenticate, requireManager, async (req, res) => {
    try {
      ...
      WHERE u.id = ? AND u.role = 'operator' AND u.manager_id = ?`, [operatorId, operatorId, managerId]);
        if (rows.length === 0) {
...
router.put('/:id', auth_1.authenticate, requireManager, async (req, res) => {
    try {
      ...
      WHERE u.id = ?`, [operatorId, operatorId]);
