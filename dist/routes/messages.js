    ...
    try {
        ...
        try {
            ...
            if (instance?.sock && instance.isReady) {
                ...
                await (0, database_1.executeQuery)('UPDATE messages SET content = ?, whatsapp_message_id = ? WHERE id = ?', [`*${operatorName}:* 📎 ${req.file.originalname} ✅`, sentMessage?.key?.id || undefined, message.id]);
...
router.delete('/:id/delete', auth_1.authenticate, async (req, res) => {
    try {
        const messageId = parseInt(req.params.id);
        if (!req.user) {
            return res.status(401).json({ error: 'Usuário não autenticado' });
        ...
        if (message.whatsapp_message_id) {
            try {
                ...
                if (instance?.sock && instance.isReady) {
                    ...
                    const contact = await Message_1.ContactModel.findById(message.contact_id);
                    if (contact) {
        ...
        const deleted = await Message_1.MessageModel.delete(messageId);
        if (!deleted) {
