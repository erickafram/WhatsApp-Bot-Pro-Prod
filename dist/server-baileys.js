...
function processMessageWithFlow(message, flowData, currentContext, isAfterClosure = false) {
    if (!flowData)
...
async function executeNodeAction(node, activeChat, managerId, dbContact, contactName, phoneNumber) {
    if (!node.data.action)
        return;
    ...
    switch (node.data.action) {
        case 'reactivate_same_operator':
            ...
            try {
                await (0, database_1.executeQuery)('UPDATE human_chats SET status = ?, updated_at = NOW() WHERE id = ?', ['pending', activeChat.id]);
                ...
                const updatedChat = await Message_1.HumanChatModel.findById(activeChat.id);
                if (updatedChat) {
                ...
                global.io.to(`manager_${managerId}`).emit('dashboard_chat_update', {
                    type: 'chat_reopened',
        ...
        case 'reset_chat_status':
            ...
            await (0, database_1.executeQuery)('UPDATE human_chats SET status = NULL, operator_id = NULL, assigned_to = NULL, updated_at = NOW() WHERE id = ?', [activeChat.id]);
            break;
        case 'reactivate_new_operator':
            ...
            try {
                await (0, database_1.executeQuery)('UPDATE human_chats SET status = ?, operator_id = NULL, assigned_to = NULL, updated_at = NOW() WHERE id = ?', ['pending', activeChat.id]);
                // Atualizar objeto na memória
                activeChat.status = 'pending';
                activeChat.operator_id = null;
                activeChat.assigned_to = null;
                // Buscar chat atualizado do banco
                const updatedChatResult = await (0, database_1.executeQuery)('SELECT * FROM human_chats WHERE id = ?', [activeChat.id]);
                ...
                global.io.to(`manager_${managerId}`).emit('customer_message', {
                    chatId: activeChat.id,
                    customerName: contactName,
                    customerPhone: phoneNumber,
    ...
    try {
        ...
        sock.ev.on('messages.upsert', async (messageUpsert) => {
            ...
            for (const msg of messages) {
                ...
                if (msg.key.fromMe)
                    continue; // Ignorar nossas próprias mensagens
...
async function markMessageAsRead(sock, msg) {
    try {
        ...
        try {
            await sock.chatModify({ markRead: true, lastMessages: [msg] }, msg.key.remoteJid);
...
async function processMessageBaileys(msg, managerId, instanceData) {
    ...
    try {
        ...
        try {
            const messageCount = await (0, database_1.executeQuery)('SELECT COUNT(*) as count FROM messages WHERE contact_id = ? AND manager_id = ?', [dbContact.id, managerId]);
        ...
        if (activeChat) {
            try {
                const lastBotMessage = await (0, database_1.executeQuery)('SELECT content FROM messages WHERE contact_id = ? AND manager_id = ? AND sender_type = "bot" ORDER BY created_at DESC LIMIT 1', [dbContact.id, managerId]);
        ...
        const customerMessage = await Message_1.MessageModel.create({
            manager_id: managerId,
...
async function saveBotMessage(response, managerId, dbContact, activeChat) {
    try {
...
async function processMediaMessage(msg, messageType, managerId) {
    try {
        const mediaDir = path_1.default.join(__dirname, '..', 'uploads', 'media');
        ...
        if (messageType === 'imageMessage') {
            ...
            if (imageMsg) {
                ...
                // Baixar imagem
                const stream = await (0, baileys_1.downloadContentFromMessage)(imageMsg, 'image');
                ...
                await (0, promises_1.pipeline)(stream, fileStream);
                mediaInfo = {
                    fileName: fileName,
            ...
            if (audioMsg) {
                ...
                const stream = await (0, baileys_1.downloadContentFromMessage)(audioMsg, 'audio');
            ...
            if (videoMsg) {
                ...
                const stream = await (0, baileys_1.downloadContentFromMessage)(videoMsg, 'video');
            ...
            if (docMsg) {
                ...
                const stream = await (0, baileys_1.downloadContentFromMessage)(docMsg, 'document');
                const originalName = docMsg.fileName || 'document';
                const extension = path_1.default.extname(originalName) || '.pdf';
...
async function createHumanChatExplicit(managerId, sender, contactName, dbContact) {
    try {
        ...
        let existingChat = await Message_1.HumanChatModel.findAnyByContact(dbContact.id);
        let humanChat;
        if (existingChat) {
            ...
            const updateQuery = `
                ...
                WHERE id = ?
            `;
            await (0, database_1.executeQuery)(updateQuery, [existingChat.id]);
            // Buscar chat atualizado
            humanChat = await Message_1.HumanChatModel.findById(existingChat.id);
        ...
        const recentMessages = await Message_1.MessageModel.findByContact(dbContact.id, 10);
        ...
        global.io.to(`manager_${managerId}`).emit('human_chat_requested', humanChatRequestData);
...
async function transferToHumanBaileys(managerId, sender, contactName, dbContact) {
    try {
            ...
            if (existingChats && existingChats.length > 0) {
                ...
                if (humanChat.status === 'finished' || humanChat.status === 'resolved' || humanChat.status === null) {
                    ...
                    try {
                        ...
                        `;
                        const updateResult = await (0, database_1.executeQuery)(updateQuery, [humanChat.id, dbContact.id, managerId]);
                        console.log(`🔄 ${updateResult.affectedRows || 0} mensagens recentes associadas ao chat reaberto ${humanChat.id}`);
                    }
                    catch (updateError) {
...
io.on('connection', async (socket) => {
    ...
    socket.on('initialize_whatsapp_baileys', async (data) => {
    ...
    socket.on('finish_human_chat', async (data) => {
        ...
        try {
            ...
            if (dbContact) {
                ...
                const anyChat = await Message_1.HumanChatModel.findAnyByContact(dbContact.id);
                if (anyChat) {
                    console.log(`🔄 Atualizando status do chat ${anyChat.id} (${anyChat.status}) para 'finished'`);
                    await (0, database_1.executeQuery)('UPDATE human_chats SET status = ?, updated_at = NOW() WHERE id = ?', ['finished', anyChat.id]);
...
//# sourceMappingURL=server-baileys.js.map